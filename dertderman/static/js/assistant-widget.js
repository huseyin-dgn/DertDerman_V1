(() => {
  ...
  const setOpen = (open) => {
    ...
    toggle.setAttribute(
      "aria-label",
      open ? "DertDerman Asistanı kapat" : "DertDerman Asistanı aç"
    );
  ...
  const makeBubble = (text, variant) => {
    const row = document.createElement("div");
    row.className = `dd-assistant__message dd-assistant__message--${variant}`;
    ...
    bubble.className = `dd-assistant__bubble dd-assistant__bubble--${variant}`;
  ...
  const appendUserMessage = (text) => {
    transcript.appendChild(makeBubble(text, "user"));
    scrollTranscript();
  };
  ...
  const showTyping = () => {
    ...
    const row = document.createElement("div");
    row.className = "dd-assistant__message dd-assistant__message--assistant";
  ...
  const renderSummary = (data) => {
    ...
    if (!complaints || typeof complaints !== "object") return null;
    if (!notifications || typeof notifications !== "object") return null;

    const total = asNonNegativeInteger(complaints.total);
    const unread = asNonNegativeInteger(notifications.unread_count);
    ...
    if (Array.isArray(complaints.status_distribution)) {
      ...
      complaints.status_distribution.forEach((item) => {
        ...
        const count = asNonNegativeInteger(item.count);
  ...
  const renderLinks = (links) => {
    ...
    links.forEach((item) => {
      ...
      const href = safeSameOriginPath(item.url);
      if (!href) return;
  ...
  const appendAssistantResponse = (payload) => {
    ...
    const links = renderLinks(payload ? payload.links : null);
    if (links) transcript.appendChild(links);
  ...
  const sendAction = async (action, label) => {
    ...
    try {
      ...
      if (!payload || payload.ok !== true) {
        appendError("Asistan yanıtı doğrulanamadı.");
        return;
      }
      ...
      removeTyping();
      appendError("Bağlantı kurulamadı. Lütfen tekrar deneyin.");
    } finally {
      setBusy(false);
    }
  };
