...
(() => {
  ...
  async function openReader(url, pushHistory = true) {
    ...
    try {
      ...
      if (!article || !article.querySelector('#reader-title')) {
      ...
      message(
        'Yazı şu anda açılamıyor',
        'Bağlantınızı kontrol edin. Yazı yayından kaldırılmış da olabilir.',
        url
      );
    } finally {
      if (current === sequence) {
  ...
  function requestClose() {
    ...
    if (window.history.state?.ddReader?.listURL === listURL) {
      window.history.back();
    }
  }

  const cleanupReaderState = ({
    restoreScroll = false,
    restoreFocus = false,
  } = {}) => {
    ++sequence;
  ...
  dialog.addEventListener('click', (event) => {
    ...
    if (event.target.closest('[data-reader-return]')) {
      event.preventDefault();
      requestClose();
    }
  });
  ...
  window.addEventListener('popstate', (event) => {
    ...
    if (state?.listURL === listURL) {
      openReader(state.url, false);
    } else if (dialog.open) {
      dialog.close();
    }
  });
