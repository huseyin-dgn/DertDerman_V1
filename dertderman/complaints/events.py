...
def record_event(
    ...
    *,
    ...
    return ComplaintEvent.objects.get_or_create(source_key=source_key, defaults=defaults)[0]


@receiver(pre_save, sender=Complaint)
def remember_status(sender, instance, raw=False, **kwargs):
    if not raw and instance.pk:
        instance._timeline_previous_status = sender.objects.filter(pk=instance.pk).values_list("status", flat=True).first()
...
def record_complaint_state(sender, instance, created, raw=False, **kwargs):
    ...
    previous = getattr(instance, "_timeline_previous_status", None)
    if previous and previous != instance.status:
        ...
        record_event(
            instance,
            event_type,
            source_key=f"complaint:{instance.pk}:status:{instance.status}:{instance.updated_at.isoformat()}",
            occurred_at=instance.updated_at,
        )
