...
def admin_shell(request):
    ...
    parents = {
        "complaint_detail": "complaint_list", "complaint_publish": "complaint_list",
    ...
    items = [{"url": reverse(f"adminx:{name}"), "label": label, "icon": icon,
              "active": name == current} for name, label, icon in SECTIONS]
    selected = next((item for item in items if item["active"]), None)
    return {"admin_navigation": items, "admin_section": selected, "admin_is_detail": route in parents}
